...
router.post('/generarCiudad', async (req, res) => {
    ...
    try {
        ...
        if (existencia.length) res.json(existencia);
...
router.post('/subirRepresentativa',async (req, res) =>{
    ...
    try{
        ...
        res.status(201).send(await admin.subirRepresentativa(data.username,data.imagen))
    }catch (error) {
        console.log("Error al realizar la operacion " + Date.now());
...
router.post('/subirFotos', async (req, res)=> {
   ...
   try {
       ...
       city_data = await admin.revisarExistenciaAdministracion(data.username);
       if(city_data.length){
           console.log(city_data);
           console.log(city_data[0].ID)
           res.status(201).send(await admin.subirFotosCiudad(data.username, data.images_data,city_data[0].ID));
       }
   } catch (error) {
       console.log(error)
...
router.post('/addPlatillo', async (req,res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            res.status(201).send(await admin.agregarPlatillo(data.username, city_data[0].ID, data.nombre, data.descripcion, data.foto));
        }
    }catch (error) {
        console.log(error)
        res.status(401).send({"error":error.message})
...
router.post('/addZonaTuristica', async (req,res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            res.status(201).send(await admin.agregarZonaTuristica(data.username, city_data[0].ID, data.nombre, data.tipo, data.descripcion, data.foto));
        }
    }catch (error) {
        console.log(error)
        res.status(401).send({"error":error.message})
...
router.post('/addFestividad', async (req,res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            res.status(201).send(await admin.agregarfestividad(data.username, city_data[0].ID, data.dia, data.mes, data.nombre, data.descripcion, data.foto));
        }
    }catch (error) {
        console.log(error)
        res.status(401).send({"error":error.message})
...
router.post('/addPersonaje', async (req,res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            res.status(201).send(await admin.agregarPersonaje(data.username, city_data[0].ID,data.nombre, data.nacimiento, data.fallecimiento, data.descripcion, data.foto));
        }
    }catch (error) {
        console.log(error)
        res.status(401).send({"error":error.message})
...
router.post('/addNota', async (req,res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            res.status(201).send(await admin.agregarNota(data.username,city_data[0].ID, data.titulo, data.descripcion,data.foto));
        }
    }catch (error) {
        console.log(error)
        res.status(401).send({"error":error.message})
...
router.post('/addEstablecimiento', async (req, res) => {
    ...
    try{
        ...
        if(city_data.length){
            console.log(city_data);
            row=(await admin.agregarEstablecimiento(city_data[0].ID, data.nombre, data.tipo, (data.telefono)?data.telefono:"", (data.correo)?data.correo:"", (data.foto)?data.foto:null));
